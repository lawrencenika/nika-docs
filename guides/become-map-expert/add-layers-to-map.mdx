...
Learn how to add data layers to your NikaCoMap using multiple methods. You can drag files from the Files panel, upload from your desktop, or use the right-click context menu.
...
- **Coordinate System**: Data should have proper spatial reference that can be converted to ESPG:3857 Mercator that NikaCoMap uses
- **File Structure**: Ensure files are not corrupted or lack of georeferencing (i.e. no lon lat coordinates at all)

## Step 2: Upload Files to Your Opened NikaCoMap
...
1. **Click Upload**: Left-click on Upload button from NikaCoMap's top bar
...
2. **Work with Vector Layers**: Learn about [Vector Layer in NikaCoMap](/guides/vector-layer-in-nikamap)
