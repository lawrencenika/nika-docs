...
## Step 6: Copy Features to a New Layer

You can copy features from one vector layer into a brand-new layer — either all features at once, or only a selected subset.

### Copy All Features
1. **Right-click** on a vector layer in the layers panel
2. Select **"Copy all features to new layer"**
3. A new layer named **"Copy of [original name]"** appears in the layer list with all features duplicated

### Copy Selected Features
1. **Select the vector layer** by clicking on it in the layers panel
2. **Select features** on the map by clicking individual features, or multi-select from the attribute table
3. **Right-click** on the layer in the layers panel
4. The menu item now shows **"Copy N feature(s) to new layer"** where N is the number of selected features
5. Click it to create a new layer containing only the selected features
6. Your selection is automatically cleared after the copy completes

<Tip>
This is useful for splitting a large dataset into smaller layers based on specific features of interest, or for isolating a subset of features to style or export independently.
</Tip>

## Step 7: Customize Layer Appearance

### Access Layer Styling
1. **Vector Layer**: Go to [Vector Layer Stylings](/guides/become-map-expert/vector-styling) to view all styles currently supported
2. **Raster Layer**: Go to [Raster Layer Stylings](/guides/become-map-expert/raster-styling) to view all visualizations currently supported

## Step 8: Export Layers from Map
...
2. **Work with Vector Layers**: Learn about [Vector Layer in NikaMap](/guides/become-map-expert/vector-styling)
3. **Collaborate**: Explore [Collaborative Mapping](/guides/become-map-expert/collaborative-mapping)
4. **Publish**: Learn how to [Publish Your Map](/guides/become-map-expert/publish-map)
