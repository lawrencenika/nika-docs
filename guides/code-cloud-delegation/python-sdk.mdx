...
Allowed status filters are `pending`, `initializing`, `running`, `cancelling`, `succeeded`, `failed`, and `cancelled`. The `limit` must be between `1` and `200`.
...
Every returned `RunHandle` exposes `parent_run_id`, which is `None` for root runs. `list_runs(parent_run_id=...)` retrieves recent runs and filters them locally.
