---
title: "Delegate functions to run in NIKA Planet Runner"
...
|--------------------|--------------|
| `run(compute=..., run_timeout_hours=..., requirements=...)` | Decorates a Python function so it can still run locally and can also be submitted to NIKA cloud compute. |
...
| `run_timeout_hours` | Optional positive integer in hours, defaulting to `1`. Bounds execution once the run has a machine. The SDK always sends a value, so omitting it submits a one-hour execution timeout rather than deferring to the server. |
| `queue_timeout_hours` | Optional positive integer in hours, at most 336 hours (14 days). Maximum time the run may wait for compute before it is failed. Omit it and the SDK leaves it out of the request entirely, so the server applies its own default of 24 hours. |
...
Per-run configuration is passed to `.submit(...)` with `RunConfig`:
...
Every returned `RunHandle` exposes `parent_run_id`, which is `None` for root runs. Handles returned from `load_run(...)` and `list_runs(...)` also carry snapshot metadata when the gateway provides it, including `name`, `machine_type`, `run_timeout_hours`, `queue_timeout_hours`, `last_status`, `created_at`, `started_at`, and `finished_at`. `list_runs(parent_run_id=...)` retrieves one page of recent runs and filters that page locally.
