...
If you have yet to install the skills, follow [Installation & Setup](/guides/custom-geoprocess-deployment/installation#step-3-initialise-ai-agent-skills-optional) to get the skills into your agent.
...
GeoEngine ships with a built-in **AI agent skill** that gives your AI coder structured knowledge of the GeoEngine workflow. If you ran `geoengine setup skills` during [installation](/guides/custom-geoprocess-deployment/installation#step-3-initialise-ai-agent-skills-optional), the following top-level skill is available to your agent:
...
- Running `geoengine workers` shows the worker "sg-pgpt-gen".

---

## Step 4: Verifying in GIS Platform

You can now test your worker following the [steps outlined](/guides/custom-geoprocess-deployment/step-by-step#step-8-gis-testing) in the guide. The AI agent
...
[these steps](/guides/custom-geoprocess-deployment/step-by-step#step-10-push-to-the-cloud) to push the next version to the cloud.
...
- **Ask for tests** — have the agent create or update `tests/geoengine.test.yaml`, then keep `geoengine test` in the development loop
