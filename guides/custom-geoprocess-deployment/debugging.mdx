...
| Column | Description |
|--------|-------------|
| **UUID** | Worker UUID used by the CLI and Docker tags |
| **NAME** | Worker name |
| **LATEST APPLIED** | Highest applied semver version plus the total number of available versions, e.g. `1.0.7 (7)` |
...
```bash
geoengine run --json --input input-file=/path/to/file.tif
```
...
geoengine delete --uuid <worker-uuid>
