...
It is ready to use as soon as the command finishes.
...
The script adds the downloader to your `PATH`, so **open a new terminal
...
The script adds the downloader to your `PATH`, so **open a new terminal
window** before running it. x86_64 only for now.

</Tab>

<Tab title="Windows">

Download `nika-planet-downloader-windows-x86_64.exe` from
[the releases page](https://github.com/NikaGeospatial/nika-planet-downloader/releases/latest),
rename it to `nika-planet-downloader.exe`, and put it somewhere on your `PATH`.
