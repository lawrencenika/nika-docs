...
- **Run Detail**: opens from any run. A fan-out **run graph** shows the run tree (a parent run and its child runs), and **Overview**, **Children**, and **Logs** tabs cover the run's status, configuration (name, run ID, timeout, resource, submitted and started times), child runs, and logs.
