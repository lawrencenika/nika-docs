...
- **All Runs**: every run submitted in the workspace, grouped by time and filterable by status (Pending, Initializing, Running, Succeeded, Failed, Cancelled). Search by name or run ID, and expand a fan-out run to see its children.
...
- **Run Detail**: opens from any run. A fan-out **run graph** shows the run tree (a parent run and its child runs), and **Overview**, **Children**, **Logs**, and **Metrics** tabs cover the run's status, configuration (name, run ID, timeout, resource, submitted and started times), child runs, logs, and resource usage.
