...
The published bundle includes open-source components (deck.gl, MapLibre GL, Apache Arrow, the GeoTIFF/raster stack, and others), each under its own permissive license (MIT, ISC, BSD, Apache-2.0). The complete inventory with full license texts ships in the package as `THIRD-PARTY-LICENSES.md` and is generated from the dependency tree on every release — those components' licenses are unaffected by OnlyMap's own terms.
