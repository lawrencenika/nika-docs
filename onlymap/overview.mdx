...
- **Routing and tracking** <sup>new in 0.6.7</sup>: `<om-layer type="Route">` draws a styled route (casing, line, origin/destination markers) from a GeoJSON LineString you already have, or resolves one from `origin`/`destination` through a pluggable `RoutingProvider` — a ~15-line adapter covers OSRM's keyless demo server, Mapbox, OpenRouteService, or your own engine. `follow="fit-route"` frames the camera on arrival. `<om-layer type="Tracking">` renders a moving entity that glides between GPS fixes with bearing-based rotation and an optional follow camera; the position feed is ordinary layer `data` (a WebSocket stream, a polled endpoint), not a separate API.
