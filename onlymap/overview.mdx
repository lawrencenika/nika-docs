...
- **34 layer types by name**: all of deck.gl's core, geo, aggregation, and mesh layers (Scatterplot, GeoJson, Arc, Path, Heatmap, Hexagon, Trips, Tile, Tile3D, Scenegraph, and more). It adds a built-in `PopupLayer` for WebGL labels at scale, and a native `COGLayer` for GeoTIFF rasters.
...
- **Formats**: JSON/GeoJSON, CSV/TSV, Apache Arrow and GeoArrow (columnar fast path, zstd IPC), Shapefile with `.dbf` attributes, KML, and **CityJSON / CityJSONSeq** (semantic 3D city models). Large parsers load on first use.
...
- **Widgets**: legend, layer switcher, basemap switcher, lighting controls, zoom controls, undo/redo, scale bar, attribution, GPU filter sliders, live Vega-Lite charts, and a drawing toolbar. You can also write custom widgets with a typed context (`ctx.data`, `ctx.stats`, `ctx.viewport`, `ctx.emit`).
