...
- **Native UI lives beside the map**, driven by the imperative handle and
  ...
  `onMeasure`.
- **Events for taps, watch tokens for continuous state.** Per-layer
  `onClick` is the tap-selection channel. `useOmMapWatch` mirrors
  continuous state (viewport, layers, data) last-write-wins — hover picks
  share the `selection` token, so a tap's trailing hover can overwrite the
  click in the same render batch if you try to drive selection UI from it.
