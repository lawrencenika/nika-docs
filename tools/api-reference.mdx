...
The Nika API provides programmatic access to all Nika services including NikaWorkspace, NikaGallery, and NikaStudio. This reference documents all available endpoints, parameters, and responses.
...
### Get Project
```http
GET /gallery/projects/{project_id}
```

### Create Project
```http
POST /gallery/projects
```
...
### Update Project
```http
PUT /gallery/projects/{project_id}
```

### Delete Project
```http
DELETE /gallery/projects/{project_id}
```
...
Need help with the API? Check out our [support page](/support) or join our [community forum](/forum). 